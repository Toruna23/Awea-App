...
export default function CampaignForm({ restaurantId }) {
  ...
  async function handleSend() {
    ...
    try {
      ...
      setResult(data.scheduled ? { scheduled: true } : data);
      setSubject("");
      setBody("");
      setHeaderImageUrl("");
      setImageUrl("");
  ...
  return (
    <div className="bg-white border border-line rounded-xl p-4 overflow-hidden">
      ...
      <div className="text-muted text-xs mb-1">When should this send?</div>
      <div className="flex gap-2 mb-2">
        <button
          onClick={() => setScheduleMode("now")}
          className={`flex-1 rounded-lg border py-2 text-xs font-bold ${scheduleMode === "now" ? "bg-amber border-amber text-white" : "border-line text-muted"}`}
        >
          Send now
        </button>
        <button
